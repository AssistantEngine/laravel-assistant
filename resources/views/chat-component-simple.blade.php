        ...
        @foreach($history as $index => $conversationItem)
            @if($conversationItem['role'] === \AssistantEngine\SDK\Models\Conversation\ConversationItem::ROLE_ASSISTANT)
                <li class="flex px-4 mb-12 ">
                    ...
                    <div>
                        ...
                        @foreach($conversationItem['messages'] as $i => $assistantMessage)
                            <div class="mt-2 text-sm space-y-3 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-100">
                                @if($i > 0)
                                    <hr class="my-4"/>
                                @endif
            ...
            @elseif($conversationItem['role'] === \AssistantEngine\SDK\Models\Conversation\ConversationItem::ROLE_USER)
                @foreach($conversationItem['messages'] as $userMessage)
                    <li class="bg-gray-50 mb-12 rounded-2xl px-4 py-6 {{$conversationItem['run_status'] === \AssistantEngine\SDK\Models\Conversation\Conversation::STATUS_CANCELLED ? 'mb-1' : ''}}">
                        <div class="mx-auto">
        ...
        @if($lastRunInFiniteState === false)
            @if ($pendingUserItem)
                @foreach($pendingUserItem['messages'] as $userMessage)
                    <li class="bg-gray-50 mb-12 rounded-2xl px-4 py-6">
                        <div class="mx-auto">
                            <div class="font-bold">{{$userName}}</div>
                            <div>
                                <p  class="text-sm mt-2">
